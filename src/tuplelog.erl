...
%format(Map = #{msg := {report, #{label := {error_logger, _}, format := Format, args := Terms}}}, _UsrConfig) ->
format(
    Map = #{msg := {report, #{label := {error_logger, _}, format := Format, args := Terms}}},
    UsrConfig
) ->
    file:write_file("/tmp/log.log", io_lib:format("Format: ~p~n", [Format]), [append]),
    file:write_file("/tmp/log.log", io_lib:format("Terms: ~p~n", [Terms]), [append]),
    file:write_file("/tmp/log.log", io_lib:format("UsrConfig: ~p~n", [UsrConfig]), [append]),
    format(
        Map#{
            msg :=
                {report, #{
...
format(#{level := Level, msg := {report, Msg}, meta := Meta}, UsrConfig) when is_map(Msg) ->
    file:write_file("/tmp/log.log", io_lib:format("###Level: ~p~n", [Level]), [append]),
    file:write_file("/tmp/log.log", io_lib:format("###Msg: ~p~n", [Msg]), [append]),
    file:write_file("/tmp/log.log", io_lib:format("###Meta: ~p~n", [Meta]), [append]),
    file:write_file("/tmp/log.log", io_lib:format("###UsrConfig: ~p~n", [UsrConfig]), [append]),
    %eel:eval(<<"Hello, <%= Name .%>!\n">>, #{'Name' => <<"World">>}).
    case maps:is_key(template, Msg) of
        true ->
            Template = maps:get(template, Msg),
            T = lists:nth(1, Template),
            M = lists:nth(2, Template),
            R = eel:eval(T, M),
            file:write_file("/tmp/log.log", io_lib:format("###R: ~p~n", [R]), [append]),
            [string:trim(R), " ", string:trim(T), " ", io_lib:format("~p", [M]), "\n"];
        false ->
            Config = apply_defaults(UsrConfig),
            NewMeta = maps:merge(Meta, #{
                level => Level,
                colored_start => Level,
                colored_end => "\e[0m"
            }),
...
format(Map = #{msg := {report, KeyVal}}, UsrConfig) when is_list(KeyVal) ->
    file:write_file("/tmp/log.log", io_lib:format("KeyVal: ~p~n", [KeyVal]), [append]),
    file:write_file("/tmp/log.log", io_lib:format("UsrConfig: ~p~n", [UsrConfig]), [append]),
    format(Map#{msg := {report, maps:from_list(KeyVal)}}, UsrConfig);
format(Map = #{msg := {string, String}}, UsrConfig) ->
    file:write_file("/tmp/log.log", io_lib:format("String: ~p~n", [String]), [append]),
    file:write_file("/tmp/log.log", io_lib:format("UsrConfig: ~p~n", [UsrConfig]), [append]),
    format(
        Map#{
            msg :=
...
format(Map = #{msg := {Format, Terms}}, UsrConfig) ->
    ...
    case is_proplist(Terms) of
        true ->
            ...
            %ReportKV1 = maps:merge(ReportKV, KV),
            ReportKV1 = ReportKV,
            format(
                Map#{msg := {report, ReportKV1}},
                UsrConfig
            );
        false ->
            format(
...
%    format_log(maps:get(template, Config), Config, Msg, NewMeta);
%format(Map = #{msg := {report, KeyVal}}, UsrConfig) when is_list(KeyVal) ->
%    %file:write_file("/tmp/test.log", io_lib:format("KeyVal: ~p~n",[KeyVal]), [append]),
%    %format(Map#{msg := {report, maps:from_list(KeyVal)}}, UsrConfig);
%    format(Map#{msg := {report, maps:from_list(tl(KeyVal))}}, UsrConfig);
%format(Map = #{msg := {string, String}}, UsrConfig) ->
%    format(Map#{msg := {report,
%                        #{unstructured_log =>
...
format_log([Key | Rest], Config, Msg, Meta, Acc) when
    is_atom(Key) orelse
        % from OTP
        is_atom(hd(Key))
->
...
format_val(colored_end, _EOC, #{colored := false}) ->
    "";
format_val(colored_end, EOC, #{colored := true}) ->
    EOC;
format_val(colored_start, _Level, #{colored := false}) ->
    "";
format_val(colored_start, debug, #{colored := true, colored_debug := BOC}) ->
    BOC;
format_val(colored_start, info, #{colored := true, colored_info := BOC}) ->
    BOC;
format_val(colored_start, notice, #{colored := true, colored_notice := BOC}) ->
    BOC;
format_val(colored_start, warning, #{colored := true, colored_warning := BOC}) ->
    BOC;
format_val(colored_start, error, #{colored := true, colored_error := BOC}) ->
    BOC;
format_val(colored_start, critical, #{colored := true, colored_critical := BOC}) ->
    BOC;
format_val(colored_start, alert, #{colored := true, colored_alert := BOC}) ->
    BOC;
format_val(colored_start, emergency, #{colored := true, colored_emergency := BOC}) ->
    BOC;
format_val(_Key, Val, Config) ->
    to_string(Val, Config).

format_time(N, #{time_offset := O, time_designator := D}) when is_integer(N) ->
    calendar:system_time_to_rfc3339(N, [
        {unit, microsecond},
        {offset, O},
        {time_designator, D}
    ]).
